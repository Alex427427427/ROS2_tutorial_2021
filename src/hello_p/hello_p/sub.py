...
class Subscriber(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(String, "string_topic", self.callback_func, 10) # create self.publisher, for data type, data channel (rostopic), callback, and queue size

    # function that will be run whenever a new message is published to the channel
    def callback_func(self, msg):
        self.get_logger().info("Received message %s" % msg.data) # prints info to terminal in a standardised ROS format
